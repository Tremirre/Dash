...
def get_dynamic_items(sup_link: str, driver: webdriver.Chrome, container: dict):
    ...
    WebDriverWait(driver, 10).until(
        expected_conditions.text_to_be_present_in_element((By.ID, "view:_id1:_id2:facetMain:_id189:holdGlosowania"), "głosowaniach")
    )
    percentage = driver.find_element_by_id("view:_id1:_id2:facetMain:_id189:holdGlosowania").text.split()[3]
    container["voting_participation"] = float(percentage[:-1])


def analyze_repr_page(sup_link: str, driver: webdriver.Chrome) -> dict:
    repr_data = {}
    dyn_thread = threading.Thread(target=get_dynamic_items, args=(sup_link, driver, repr_data))
    dyn_thread.start()
    repr_page = requests.get(GOV_BASE_ADDRESS + sup_link)
    parsed = bs4.BeautifulSoup(repr_page.content, "html.parser")
    base_data = parsed.find("div", {"class":"partia"}).find("ul", {"class":"data"}).find_all("li")
    cv_data = parsed.find("div", {"class":"cv"}).find("ul", {"class":"data"}).find_all("li")
